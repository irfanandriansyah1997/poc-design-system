...
ESLINT_CONFIG=$ROOT_DIR/.eslintrc.cjs
...
do
  ...
  RETVAL=$?
  if [ "$RETVAL" -eq "0" ] && [ "$FILE" != ".eslintrc.js" ] && [ "$FILE" != "craco.config.js" ]
  then
      echo "$FILE"
...
if [ "$JS_STAGED_FILES" != "" ]; then
    $ESLINT_BIN -o ./tmp/eslint.txt --no-color --max-warnings 0 -c $ESLINT_CONFIG $JS_STAGED_FILES
    ESLINT_RETVAL=$?
fi
